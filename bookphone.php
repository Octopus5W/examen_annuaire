...
if (!isset($_SESSION['login']) || empty($_SESSION['login'])) {
    ...
    $contacts = get_all_contacts($_SESSION['login']['id']);
}
?>
...
<header class="flex items-center w-full gap-4">
    <div class="flex  w-full">
        <a class="flex flex-row items-center" href="myprofile.php">
            <div class="flex flex-col">
                <b class="text-cyan-950 block">Bienvenue <?php echo $_SESSION["login"]["username"];?></b>
                <span class=" text-sm"><?php echo explode(" ",$_SESSION["login"]["login_date"])[0];?></span>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="36px" height="36px">
    ...
    <a class="text-cyan-950 hover:text-cyan-900 block font-semibold" href="?action=logout">Déconnexion</a>
</header>
<main>
        <div class="flex justify-between pb-2">
            <span class="font-semibold">Nombre de contact(s) : <?php echo count($contacts) ?></span>
            <a href="add_contact.php">
                <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M5.67157 5.67157C4.5 6.84315 4.5 8.72876 4.5 12.5V23.5C4.5 27.2712 4.5 29.1569 5.67157 30.3284C6.84315 31.5 8.72876 31.5 12.5 31.5H23.5C27.2712 31.5 29.1569 31.5 30.3284 30.3284C31.5 29.1569 31.5 27.2712 31.5 23.5V12.5C31.5 8.72876 31.5 6.84315 30.3284 5.67157C29.1569 4.5 27.2712 4.5 23.5 4.5H12.5C8.72876 4.5 6.84315 4.5 5.67157 5.67157ZM17 10.5V17H10.5V19H17L17 25.5H19V19H25.5V17H19V10.5H17Z" fill="#222222" />
                </svg>
            </a>
        </div>
        <div class="grid grid-cols-2 gap-4 ">
            <?php foreach ($contacts as $contact) : ?>
                <div class="bg-gray-50 flex flex-col items-center p-4 gap-4 shrink-0  rounded-lg ">
                    <a class="bg-gray-300 hover:bg-gray-200 rounded-full w-full h-full text-center py-4 font-semibold uppercase" href="profile?id=<?php echo $contact['id'] ?>"><span><?php echo $contact["firstname"] ?></span><span><?php echo $contact["lastname"] ?></span></a>
                    <span class="font-semibold"><?php echo $contact["phone_number"] ?></span>
                </div>
            <?php endforeach; ?>
        </div>
</main>
...
if (isset($_GET['error']) && !empty($_GET['error'])) {
    echo '<span class="text-red-500 block py-4 font-semibold  text-center">' . $_GET['error'] . '</span>';
}
?>
